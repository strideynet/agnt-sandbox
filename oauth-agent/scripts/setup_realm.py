#!/usr/bin/env python3
"""
Keycloak Realm Setup for OAuth Agent

This script creates and configures a Keycloak realm for demonstrating:
1. OAuth2 Authorization Code Flow with user consent
2. Token Exchange for agent impersonation

The realm includes:
- consent-ui-client: Confidential client for user authentication and consent
- oauth-agent-client: Confidential client for token exchange
...
def main():
    ...
    REALM_NAME = "agent-demo"

    print("=" * 50)
    print("Keycloak OAuth Agent Realm Setup")
    print("=" * 50)
    ...
    # Create consent-ui-client (for OAuth2 authorization code flow)
    # This is the "source" client that users authenticate with
    consent_ui_config = {
    ...
    consent_ui_id = admin.create_client(REALM_NAME, consent_ui_config)
    if not consent_ui_id:
        print("Failed to create consent-ui-client")
        sys.exit(1)

    # Create oauth-agent-client (for token exchange)
    # This is the "target" client that can exchange tokens
    agent_client_config = {
    ...
    agent_client_id = admin.create_client(REALM_NAME, agent_client_config)
    if not agent_client_id:
        print("Failed to create oauth-agent-client")
        sys.exit(1)

    # Configure token exchange by adding audience mapper
    print("\n" + "=" * 50)
    print("Configuring Token Exchange")
    print("=" * 50)

    # Add oauth-agent-client to consent-ui-client's audience
    # This allows tokens from consent-ui-client to be exchanged by oauth-agent-client
    admin.add_audience_mapper(REALM_NAME, consent_ui_id, "oauth-agent-client")
    ...
    print()
    print("Clients:")
    print(f"  consent-ui-client:")
    print(f"    Client ID: consent-ui-client")
    print(f"    Client Secret: agent-secret-change-in-production")
    print(f"    Purpose: User authentication via OAuth2 authorization code flow")
    print()
    print(f"  oauth-agent-client:")
    print(f"    Client ID: oauth-agent-client")
    print(f"    Client Secret: agent-client-secret-change-in-production")
